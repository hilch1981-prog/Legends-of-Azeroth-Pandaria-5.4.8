...
| Mana Tea | 115294 / 123761 | `// 115294 - Mana Tea` channel sizes duration from current stack count and consumes `115867` on periodic ticks; `// 123761 - Mana Tea` is the glyphed instant variant with an explicit two-stack `CheckCast` gate | active variants confirmed; live spellbook override behavior pending |
...
- Target-core `spell_monk_fortifying_brew` is an `AuraScript` registered on `120954`; this proves that `120954` is an effect/aura in this core, not that it is the learned player cast. Independent MoP-era evidence from the original expansion and current MoP Classic data consistently identifies `115203` as the player Fortifying Brew cast. Because the target repository does not itself expose `115203` in searchable source, PlayerBot treats that mapping as strongly corroborated but still runtime/DBC-pending rather than fully target-core-confirmed.
- Fortifying Brew resolution is now fail-closed and Monk-local: if `HasActiveSpell(115203)` is true, PlayerBot uses that exact cast ID. Otherwise it accepts the ordinary `"fortifying brew"` name-resolved ID only when that ID is itself active and is **not** known aura/script `120954`. The action never executes `120954` as a player cast, and usefulness recognizes either `120954` or `115203` as an already-active defensive aura until runtime settles the exact visible aura mapping.
...
- Normal Mana Tea `115294` sets its channel duration from the current `115867` stack count in `HandleApply`; each periodic tick consumes one stack. There is no two-stack target-core gate on the normal channel, so a one-stack channel is valid. The glyphed Mana Tea `123761` is an instant spell whose `CheckCast` explicitly requires at least two `115867` stacks and whose cast handler consumes two stacks.
...
`PlayerbotAI::CanCastSpell(std::string, ...)` and `PlayerbotAI::CastSpell(std::string, ...)` both consume this `"spell id"` value, so the ambiguity affects both preflight and execution. The fix is intentionally Monk-local: Guard checks `HasActiveSpell(123402)` first and `HasActiveSpell(115295)` second; Mana Tea checks `HasActiveSpell(123761)` first and `HasActiveSpell(115294)` second; Fortifying Brew checks exact active `115203` first and rejects known aura/script `120954` if it ever appears as the name-resolved candidate. Each specialized action then uses the exact accepted ID for its own legality/execution path. Global `SpellIdValue` is not changed because its ordering behavior may be relied upon by unrelated legacy/ranked spell paths.

This does not yet claim the live glyph/DBC paths are fully validated. If the core exposes only the normal spell in `PlayerSpellMap` and performs an override elsewhere at cast time, the normal fallback remains available. If it exposes an override as an active spellbook replacement, Guard and Mana Tea select it deterministically. Fortifying Brew accepts an alternative name-resolved ID only if it is an active learned spell and is not `120954`. Guard, Mana Tea, and Fortifying Brew remain runtime verification items.
...
- Fortifying Brew — verify build-18414 `HasActiveSpell(115203)`, the cast result, cooldown, and resulting visible aura. Target core directly scripts aura/effect `120954`; independent MoP-era sources strongly corroborate player cast `115203`, but the target DBC/runtime still must make the final determination. The implementation now prefers active `115203`, rejects `120954` as a cast, and otherwise fails closed unless a different name-resolved ID is itself active.
